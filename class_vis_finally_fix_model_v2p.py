...
class Classifier_Module(nn.Module):
    ...
    def __init__(self, dilation_series, padding_series, num_classes):
        ...
        self.classifier_b = nn.Conv2d(16, num_classes, kernel_size=1, stride=1, bias=False)
        self.classifier_s = nn.Conv2d(16, num_classes, kernel_size=1, stride=1, bias=False)
        self.classifier_s1 = nn.Conv2d(256, num_classes, kernel_size=1, stride=1, bias=False)
        self.classifier_p = nn.Conv2d(256, num_classes, kernel_size=1, stride=1, bias=False)
        self.classifier_e = nn.Conv2d(256, num_classes, kernel_size=1, stride=1, bias=False)
        self.classifier_l2468 = nn.Conv2d(64, num_classes, kernel_size=1, stride=1, bias=False)
    ...
    def forward(self, x, l2, l4, l6, l8):
        ...
        visout_1 = self.classifier_b(outcam1)
        # -->16*(64*64)
        ...
        # visout_2 = s
        visout_2 = self.classifier_s(outcam5)
        ##
        outcam1 = self.upchannel(outcam5)
        # visout_3 = S1
        visout_3 = self.classifier_s1(outcam1)
        outcam1 = torch.cat((outcam,outcam1),1)
        # 512*64*64 --> 256 *64*64
        outcam1 = self.downchannel(outcam1)
        # visout_4 = p
        visout_4 = self.classifier_p(outcam1)
        # jinzita kongdong
        ...
        # junyun
        l2 = self.l2(l2)
        vl2 = self.classifier_l2468(l2)
        l4 = self.l4(l4)
        vl4 = self.classifier_l2468(l4)
        l6 = self.l6(l6)
        vl6 = self.classifier_l2468(l6)
        l8 = self.l8(l8)
        vl8 = self.classifier_l2468(l8)
        l = torch.cat([l2,l4,l6,l8], dim=1).type(torch.FloatTensor)
        vis_E=self.classifier_e(l)
        # 256 *64*64 --> 16 *64*64
        ...
        lo = self.lo(lo.cuda())
        vis_V = self.classifier(lo)
        lo = self.sigmoid(lo)
        vis_U = self.classifier(lo)
        out = self.classifier(lo)

        return out_last,out,visout_0,visout_1,visout_2,visout_3,visout_4,vl2,vl4,vl6,vl8,vis_E,vis_V,vis_U
