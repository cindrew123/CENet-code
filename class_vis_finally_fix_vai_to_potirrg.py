...
from deeplab.class_vis_finally_fix_model_v2p import diceCoeffv2, SoftDiceLoss, Res_Deeplab, Discriminator, Discriminator_Down, Discriminator_NumClass_2
...
def test(model, step, testloader):
    ...
    for index, batch in enumerate(testloader):
        ...
        size = size[0].numpy()
        _, output,_,_,_,_,_,_,_,_,_,_,_,_ = model(Variable(image, volatile=True).cuda())
...
def main():
    ...
    for step in range(args.num_steps):
        ...
        out_predict_train, output_train,_,_,_,_,_,_,_,_,_,_,_,_ = model(images_train)
        out_last_test, output_test,_,_,_,_,_,_,_,_,_,_,_,_ = model(images_test)
        ...
        if (step + 1) % args.save_pred_every == 0:
            ...
            torch.save(model.state_dict(), osp.join(args.snapshot_dir,
                                                    'class_vis_target_v2p_change_numclass_2_softmax_bce_' + str(
                                                        step + 1) + '.pth'))
            print 'taking critic snapshot ...'
            torch.save(critic.state_dict(), osp.join(args.snapshot_dir,
                                                     'class_vis_v2p_critic_change_numclass_2_softmax_bce_' + str(
                                                         step + 1) + '.pth'))
